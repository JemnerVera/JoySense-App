...
// USER SELECTOR
// ============================================================================
// Componente para seleccionar usuarios con búsqueda inteligente
// Similar a LocationSelector pero para usuarios
...
interface UserOption {
  ...
  label: string; // "firstname lastname - login" para mostrar en el dropdown
  usuarioid: number;
...
export const UserSelector: React.FC<UserSelectorProps> = ({
  ...
  const loadOptions = useCallback(async (inputValue: string): Promise<UserOption[]> => {
    ...
    try {
      const results = await JoySenseService.searchUsers(inputValue);
      return results.map((u: any) => ({
        value: u.usuarioid,
        label: u.label || `${u.firstname || ''} ${u.lastname || ''} - ${u.login || ''}`.trim(),
    ...
    if (value && !selectedLabel) {
      ...
      const searchUser = async () => {
        try {
          ...
          const results = await JoySenseService.searchUsers(value.toString());
  ...
  const handleSelectSuggestion = (option: UserOption) => {
