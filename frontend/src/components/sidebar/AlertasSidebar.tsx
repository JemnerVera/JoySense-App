      ...
      <div className={`h-full overflow-y-auto ${isExpanded ? 'custom-scrollbar' : 'scrollbar-hide'}`}>
        <div className="py-4">
          <nav className="space-y-1">
            ...
            {/* ALERTA_REGLA */}
            <button
              onClick={() => onTabChange?.('alertas-alerta_regla-status')}
              className={`w-full flex items-center p-3 rounded transition-colors ${
                isExpanded ? 'gap-3' : 'justify-center'
              } ${
                activeTab?.startsWith('alertas-alerta_regla')
                  ? 'bg-red-500 text-white'
                  : 'text-red-300 hover:text-white hover:bg-red-600'
              }`}
            >
              ...
              {isExpanded && (
                <span className="text-sm font-medium tracking-wider">{t('alerts.rule_alert').toUpperCase()}</span>
              )}
            </button>
          </nav>
        </div>
