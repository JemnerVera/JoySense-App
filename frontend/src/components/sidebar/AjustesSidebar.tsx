 ...
 * Muestra las opciones CONF. BÁSICAS y CONF. AVANZADAS
 */
...
interface AjustesSidebarProps {
  selectedSection: 'basicas' | 'avanzadas';
  onSectionSelect: (section: 'basicas' | 'avanzadas') => void;
  isExpanded: boolean;
  onMouseEnter: () => void;
  onMouseLeave: () => void;
}
...
const AjustesSidebar: React.FC<AjustesSidebarProps> = ({
  ...
  const sections = [
    ...
    {
      id: 'avanzadas' as const,
      label: 'CONF. AVANZADAS',
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
