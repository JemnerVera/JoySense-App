...
interface ParametrosOperationsSidebarProps {
  isExpanded: boolean;
  onMouseEnter: () => void;
  onMouseLeave: () => void;
...
const ParametrosOperationsSidebar: React.FC<ParametrosOperationsSidebarProps> = ({
  isExpanded,
  onMouseEnter,
  onMouseLeave,
  selectedTable,
  ...
  const getAvailableOperations = () => {
    ...
    if (!config) return allOperations.filter(op => op.id !== 'massive');
  ...
  const getTableDisplayName = (tableValue: string): string => {
    const config = getTableConfig(tableValue);
    return config?.displayName || tableValue;
  };
...
export default ParametrosOperationsSidebar;
