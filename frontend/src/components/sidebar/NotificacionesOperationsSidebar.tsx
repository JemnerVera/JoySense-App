...
const NotificacionesOperationsSidebar: React.FC<NotificacionesOperationsSidebarProps> = ({
  ...
  const availableOperations = useMemo(() => {
    console.log('🔍 [NotificacionesOperationsSidebar] Filtrando operaciones', {
      selectedTable,
      permissionsLoading,
      permissions,
    ...
    const filtered = allOperations.filter(op => {
      ...
      if (op.id === 'insert' && !config?.allowInsert) {
        console.log('❌ [NotificacionesOperationsSidebar] Operación filtrada (config):', op.id);
        return false;
      }
      ...
      if (permissionsLoading) {
        console.log('⏳ [NotificacionesOperationsSidebar] Permisos cargando, mostrando operación:', op.id);
        return true;
      }
      
      // Verificar permisos del usuario (solo cuando ya se cargaron)
      if (op.requiredPermission === 'ver' && !permissions.puede_ver) {
        console.log('❌ [NotificacionesOperationsSidebar] Operación filtrada (sin permiso ver):', op.id);
        return false;
      }
      if (op.requiredPermission === 'insertar' && !permissions.puede_insertar) {
        console.log('❌ [NotificacionesOperationsSidebar] Operación filtrada (sin permiso insertar):', op.id);
        return false;
      }
      if (op.requiredPermission === 'actualizar' && !permissions.puede_actualizar) {
        console.log('❌ [NotificacionesOperationsSidebar] Operación filtrada (sin permiso actualizar):', op.id);
        return false;
      }
      
      console.log('✅ [NotificacionesOperationsSidebar] Operación permitida:', op.id);
      return true;
    });

    console.log('📋 [NotificacionesOperationsSidebar] Operaciones disponibles:', filtered.length, filtered.map(op => op.id));
    return filtered;
